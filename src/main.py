...
def load_config():
    ...
    if not raw:
        raise RuntimeError("Brak SEARCH_CONFIG_JSON.")
    cfg = json.loads(raw)
...
def score_listing(item, median_ppm2):
    ...
    positive = [
        "do remontu", "do odświeżenia", "pilnie", "do negocjacji",
        "bezpośrednio", "bez pośredników", "balkon", "winda", "blok",
    ]
...
def fmt_offer(item, score, reasons, search_name, median_ppm2):
    ...
    label = "🔥 MOCNA OKAZJA" if score >= 75 else "👀 WARTO SPRAWDZIĆ"
...
def run(dry_run=False):
    ...
    stats = load_stats()
    stats["runs"] += 1
    stats["checked"] += checked_total
    stats["new_offers"] += len(newly_seen)
    stats["alerts"] += min(len(alerts), 20)
